...
def assign_colours_to_clusters(dataframes, max_level):
    ...
    for i, (_, cluster_row) in enumerate(top_level_clusters.iterrows()):
        ...
        _assign_descendant_colors(
            dataframes, max_level, cluster_id, base_colour, colour_assignments
        )

    return colour_assignments


def _assign_descendant_colors(
    dataframes, level, cluster_id, base_colour, colour_assignments
):
    ...
    if child_cluster_ids:
        ...
        for child_index, child_id in enumerate(child_cluster_ids):
            ...
            _assign_descendant_colors(
                dataframes, level - 1, child_id, child_colour, colour_assignments
            )
    ...
    for _, cluster_row in df.iterrows():
        ...
        with st.container():
            ...
            with col2:
                ...
                if "category" in cluster_row.index and pd.notna(
                    cluster_row["category"]
                ):
            ...
            if st.button(
                "Explore →",
                key=f"cluster_{level}_{cluster_id}_{category}",
...
def display_full_example(
    linked_examples_df: pd.DataFrame, examples_df: pd.DataFrame, example_id: str
):
    matching_examples = linked_examples_df[
        linked_examples_df["example_id"] == example_id
    ]
    ...
    summary = (
        matching_summaries["summary"].iloc[0]
        ...
        .agg(
            {
                "size": [
                    "count",
                    "sum",
                ]  # count = # clusters, sum = total conversations
            }
        )
        .round()
    )
...
def main():
    ...
    st.markdown(
        "See 2990 customer support conversations organized into hierarchical clusters. You can navigate from 5 high-level topic areas down through 15 mid-level clusters, then to 125 specific clusters, and finally to conversation summaries and full Chat LangChain threads."
    )
    ...
    for level_key in ["level_0", "level_1", "level_2"]:
        if level_key in dataframes:
            ...
            if size_col in dataframes[level_key].columns:
                dataframes[level_key] = dataframes[level_key].sort_values(
                    size_col, ascending=False
                )
    ...
    with st.sidebar:
        ...
        for category, conv_count in categories.items():
            ...
            st.markdown(
                f"**{category}**: {conv_count:,} conversations ({percentage:.1f}%)"
            )

        # Add insights
        st.divider()
        st.markdown("### Quick Insights")  # TODO
        st.info("[can put anything else we'd want pinned/to see instantly here]")
        st.success("[like this]")
        st.warning("[and this]")
    ...
    if st.session_state.view_mode == "full_example":
        display_full_example(
            dataframes["linked_examples"],
            dataframes["examples"],
            st.session_state.selected_example_id,
        )
