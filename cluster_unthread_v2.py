...
def cluster_category_examples(category, category_examples, id_offset):
    ...
    for level in range(1, levels):
        ...
        kmeans_nbh = KMeans(n_clusters=k_neighborhoods, random_state=42, n_init=10, max_iter=300)
    ...
    <scratchpad>"""
        ...
        if len(proposed) <= target_clusters:
            ...
            deduplicating_user_prompt = DEDUPLICATE_CLUSTERS_INSTR.format(cluster_text=cluster_text, target_clusters=target_clusters, clusters_per_neighborhood=clusters_per_neighborhood, criteria=criteria, min_cpn=int(0.5 * target_clusters), max_cpn=int(1.5 * target_clusters))
...
<scratchpad>"""
        ...
        for cluster_id, cluster_info_item in current_clusters.items():
            assign_user_prompt = ASSIGN_CLUSTER_INSTR.format(
                ...
                specific_cluster=cluster_info_item
            )
...
<scratchpad>"""
        ...
        for hl_id, (hl_name, member_cluster_ids) in enumerate(cluster_groups.items()):
            ...
            for cluster_id in member_cluster_ids:
                ...
                total_size += cluster_info_item.get("size", cluster_info_item.get("total_size", 1))
...
for category, category_examples in examples_by_category.items():
    ...
    print(f"Using K={int(k * len(category_examples) / total_examples)} clusters for {len(category_examples)} summaries")
