...
def load_embeddings_and_clusters(
    experiment_name=None, dataset_name="open-clio-data-test", regenerate_embeddings=True
):
    """
    ...
    for example in examples:
        if "cluster_id" in example.outputs:
            ...
            cluster_names.append(
                example.outputs.get(
                    "cluster_name", f"Cluster {example.outputs['cluster_id']}"
                )
            )
    ...
    if embeddings is None and regenerate_embeddings:
        ...
        response = openai.embeddings.create(model=embedding_model, input=summaries)
...
def create_umap_projection(
    embeddings, n_neighbors=15, min_dist=0, metric="cosine", random_state=42
):
    """
...
def plot_clusters(
    ...
    for i, cluster_id in enumerate(unique_clusters):
        ...
        if cluster_names is not None:
            ...
            if cluster_name_list:
                ...
                label = (
                    f"{cluster_id}: {cluster_name[:40]}..."
                    if len(cluster_name) > 40
                    else f"{cluster_id}: {cluster_name}"
                )
            else:
        ...
        ax_main.scatter(
            cluster_points[:, 0], cluster_points[:, 1], c=[colors[i]], alpha=0.7, s=25
        )
    ...
    if show_legend:
        ...
        for i, info in enumerate(legend_info):
            ...
            display_name = (
                info["name"][:35] + "..." if len(info["name"]) > 35 else info["name"]
            )

            legend_text += f"{info['id']}: {display_name}\n"
            legend_text += (
                f"    ({info['count']} points, {info['percentage']:.1f}%)\n\n"
            )
    ...
    for info in sorted(legend_info, key=lambda x: x["id"]):
        print(
            f"Cluster {info['id']}: {info['count']} points ({info['percentage']:.1f}%) - {info['name'][:50]}..."
        )
...
def create_separate_legend(cluster_labels, cluster_names, colors, save_path=None):
    ...
    for i, cluster_id in enumerate(unique_clusters):
        ...
        if cluster_names is not None:
            ...
            cluster_name = (
                cluster_name_list[0] if cluster_name_list else "No name available"
            )
        else:
            cluster_name = "No name available"
...
def create_interactive_plot(
    embedding_2d, cluster_labels, summaries, cluster_names=None
):
    """
        ...
        df = pd.DataFrame(
            {
                ...
                if cluster_names
                else [f"Cluster {c}" for c in cluster_labels],
            }
        )

        # Create hover text
        df["hover_text"] = df.apply(
            lambda row: f"Cluster {row['cluster']}<br>{row['cluster_name']}<br><br>{row['summary'][:100]}...",
            axis=1,
        )
...
def main(experiment_name=None, dataset_name="open-clio-data-test"):
    ...
    plot_clusters(embedding_2d, cluster_labels, cluster_names, save_path=save_path)
    ...
    interactive_fig = create_interactive_plot(
        embedding_2d, cluster_labels, summaries, cluster_names
    )
    if interactive_fig:
