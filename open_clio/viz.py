...
st.set_page_config(
    page_title="Clustering Explorer",
    layout="wide",
...
class ClusteringExplorer:
    ...
    def assign_colors_to_clusters(self) -> Dict[Tuple[int, int], str]:
        ...
        top_level_data = self.data[f'level_{self.max_level}'] # start with top level clusters
        ...
        for i, (_, cluster_row) in enumerate(top_level_data.iterrows()):
            ...
            self._assign_child_colors(self.max_level, cluster_id, base_color, color_assignments)  # Recursively assign colors to child clusters
...
def init_session_state(explorer: ClusteringExplorer):
    ...
    if 'view_mode' not in st.session_state:
        st.session_state.view_mode = 'partitions' if explorer.config.get("partitions") else 'clusters'
    ...
    for _, cluster_row in df.iterrows():
        ...
        with st.container():
            ...
            with col2:
                ...
                if partition_col:
                    st.markdown(f"*{partition_col.title()}: {cluster_row[partition_col]}*")
...
def display_parent_stack():
    ...
    if st.button("← Back"):
        if st.session_state.navigation_path:
            ...
            if st.session_state.navigation_path:
                ...
                st.session_state.view_mode = 'examples' if last_step['level'] == 0 else 'clusters'
            else:
                # Back to top level
                st.session_state.current_level = None
                st.session_state.selected_cluster = None
                st.session_state.view_mode = 'partitions' if st.session_state.get('has_partitions') else 'clusters'
        st.rerun()
    ...
    if st.session_state.navigation_path:
        ...
        if current_data is not None:
            current_cluster = current_data[current_data['cluster_id'] == current_cluster_id]
            if len(current_cluster) > 0:
                current_cluster = current_cluster.iloc[0]
    ...
    if level_0_data is not None:
        ...
        if len(current_cluster) > 0:
            ...
            for col in ['category', 'partition']:
                if col in current_cluster.columns and pd.notna(current_cluster.iloc[0].get(col)):
                    partition_col = col
                    break
            
            if partition_col:
                st.markdown(f"**{partition_col.title()}:** {current_cluster.iloc[0][partition_col]}")
        ...
        with st.container():
            ...
            with col1:
                ...
                for col in example_row.index:
                    if col not in ['example_id', 'summary', 'base_cluster_id', 'base_cluster_name'] and pd.notna(example_row[col]):
                        st.write(f"**{col.title()}:** {example_row[col]}")
            
            with col2:
                if 'full_example' in example_row.index and pd.notna(example_row['full_example']):
...
def show_cluster_previews_in_sidebar(explorer: ClusteringExplorer):
    ...
    for level, level_data in available_levels:
        ...
        if len(level_data) > preview_count:
            ...
            if st.button(f"Show all {len(level_data)} clusters ↓", key=f"show_all_{level}"):
                ...
                if level == max_level:
                    ...
                    st.info(f"Navigate through the hierarchy to see Level {level} clusters")
                st.rerun()
...
def display_partition_overview(explorer: ClusteringExplorer):
    ...
    partition_stats = level_0_data.groupby(partition_col).agg({
        'size': ['count', 'sum']
    }).round()
    ...
    for i, (partition_name, stats) in enumerate(partition_stats.iterrows()):
        with cols[i % 2]:
            with st.container():
                ...
                if st.button(f"Explore {partition_name} →", key=f"partition_overview_{i}", use_container_width=True):
...
def display_partition_clusters(explorer: ClusteringExplorer, partition_name: str):
    ...
    display_cluster_table(partition_data, 0, st.session_state.color_assignments, explorer)
    ...
    with st.sidebar:
        ...
        if level_0_data is not None:
            ...
            if partitions and level_0_data is not None:
                ...
                st.markdown("*Partitions group the data by main topic areas or categories*")
                ...
                if partition_col:
                    partition_counts = level_0_data.groupby(partition_col)['size'].sum().sort_values(ascending=False)
                    for partition, count in partition_counts.items():
                        percentage = (count / total_items) * 100
                        st.markdown(f"**{partition}**: {count:,} items ({percentage:.1f}%)")
            st.divider()
