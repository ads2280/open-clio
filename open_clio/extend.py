...
async def process_single_examples(example, existing_data, expected_partition=None):
    ...
    level_0_result = await assign_to_base_cluster(
        example, existing_data, partition, summary
    )
    ...
    full_assignments = await assign_higher_levels(
        combined_result, existing_data, partition
    )
    ...
    print(
        f"Successfully assigned example {example.id} (partition: {partition}, level0: {level_0_result['level_0_cluster_name']}, level1+: see csvs)"
    )
...
def load_hierarchy(save_path):
    ...
    if not os.path.exists(combined_csv_path):
        ...
        print(
            "Please run generate.py first to create the initial clusters before extending."
        )
        exit(1)
    ...
    for partition in partitions:
        ...
        level_columns = [
            col
            for col in partition_examples.columns
        ...
        for level in range(max_level + 1):
            ...
            cluster_names_by_partition[partition][level_key] = partition_clusters[
                ["cluster_id", "name"]
            ].to_dict("records")
...
async def determine_partition_and_summarize(example, existing_data, partitions=None):
    ...
    if partitions is None:
        ...
        available_partitions = (
            list(partitions.keys()) if isinstance(partitions, dict) else partitions
        )
    prompt = PARTITION_AND_SUMMARIZE.format(
        available_partitions=available_partitions, example=example
    )
    ...
    try:
        ...
        if partition not in available_partitions:
            logger.error(
                f"Invalid partition '{partition}' returned for example {example.id}. Available: {available_partitions}"
            )
            return None
    ...
    try:
        ...
        for cluster in level_0_clusters:
            if cluster["name"] == level_0_cluster_name:
                ...
                print(
                    f"Assigned example {example.id} to level 0 cluster {level_0_cluster_name} (ID: {level_0_cluster_id})"
                )
                break

        if level_0_cluster_id is None:
            logger.error(
                f"Could not find cluster ID for name '{level_0_cluster_name}' in partition '{partition}'"
            )
            return None
...
async def assign_higher_levels(
    level_0_assignment, partition_clusters, partition_name="Default"
):
    assignments = {
        "level_0": {
    ...
    for level in range(1, max_level + 1):
        ...
        examples_with_this_level_n = combined_df[
            combined_df[previous_level_col] == current_assignment_id
        ]
        if len(examples_with_this_level_n) == 0:
            warnings.warn(
                f"No examples found for level {level} cluster {current_assignment_id}"
            )
            break
        ...
        available_clusters = partition_clusters["cluster_names_by_partition"][
            partition_name
        ][level_key]
        valid_clusters = [
            c for c in available_clusters if c["cluster_id"] in valid_cluster_ids
        ]

        assert len(valid_clusters) == 1, (
            f"Multiple level {level_key} clusters found for {current_assignment_id}: {valid_clusters}"
        )
        chosen_cluster = valid_clusters[0]
        print(
            f"Assigning level {level} cluster {chosen_cluster['name']} to {current_assignment_id} (only option)"
        )
...
def extend_results(new_assignments, save_path):
    ...
    print(
        f"Extending results with {len(new_assignments)} new assignments, in {save_path}"
    )
    ...
    for assignment in new_assignments:
        ...
        for level in range(1, max_level + 1):
            ...
            if level_key in assignments_dict:
                ...
                row_data[f"{level_key}_name"] = assignments_dict[level_key][
                    "cluster_name"
                ]
        ...
        if max_level >= 0:  # does this work if we ONLY have base clusters
            ...
            if top_level_key in assignments_dict:
                row_data["top_cluster_id"] = assignments_dict[top_level_key][
                    "cluster_id"
                ]
                row_data["top_cluster_name"] = assignments_dict[top_level_key][
                    "cluster_name"
                ]
    ...
    print(
        f"Updated combined.csv: added {len(new_rows)} new rows, total {len(updated_combined)} rows"
    )
...
def update_cluster_files(cluster_updates, save_path):
    ...
    for level, cluster_counts in updates_by_level.items():
        ...
        if not os.path.exists(original_level_file):
            warnings.warn(
                f"Level {level} cluster file not found: {original_level_file}, cannot update {len(cluster_counts)} level {level} clusters"
            )
            continue
        ...
        for cluster_id, additional_count in cluster_counts.items():
            ...
            if mask.any():  # update size
                if level == 0:
                    ...
                    print(
                        f"  Updated Level {level} cluster {cluster_id}: {current_size} -> {new_size} examples"
                    )
                else:
                    ...
                    if "total_size" in df.columns:
                        ...
                        print(
                            f"  Updated level_{level} cluster {cluster_id}: total_size {current_total_size} -> {new_total_size} examples"
                        )
                    else:
                        warnings.warn(
                            f"Level {level} cluster file missing 'total_size' column"
                        )
            else:
                warnings.warn(
                    f"Cluster {cluster_id} not found in level_{level}_clusters.csv"
                )
