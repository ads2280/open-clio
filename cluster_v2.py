...
async def generate_single_summary(example, semaphore, counter, total_examples, summary_prompt):
...
async def summarize_all(examples: list, dataset_name: str, summary_prompt: str, max_concurrent: int = 5):
    ...
    start_time = time.time()
    tasks = [
        generate_single_summary(example, semaphore, counter, total_ex, summary_prompt) for example in examples
    ]
...
def cluster_category_examples(category, category_examples, total_examples, id_offset, hierarchy):
    ...
    level_sizes = hierarchy[1:]   # TODO - decide if we skip the first element (base k) since we already have n_base clusters
...
def generate_clusters(
    ...
    asyncio.run(summarize_all(examples, dataset_name, summary_prompt, max_concurrent=5))
    time.sleep(30) # TODO fix
    
    # Reload data to get the updated examples with summaries
    examples = list(client.list_examples(dataset_name=dataset_name, limit=sample if sample else None))

    # atp, should have dealt with partitions or set category to ALL TODO
    ...
    skip_categories = ["Error extracting category", "<UNKNOWN>"] # one of these should always be what we provide as a 'misc' category TODO
...
if __name__ == "__main__":
    ...
    dataset_name = "ds-granular-pseudoscience-68"  # Replace with actual dataset name
    save_path = "./clustering_results"  # Directory to save results
    summary_prompt = SUMMARIZE_INSTR  # placeholder is prompt from prompts.py, replace with actual summary prompt
    hierarchy = [5, 1]  # Example hierarchy: [base_clusters, intermediate, top_clusters]
    # this would still create 7 top level clusters, not 1. assert somehwere that num_categories = ktop?
    
    # Optional params
    partitions = None  # right now this doesn't to anything. ideally should be injected in the summary prompt to force structured output. 
                        # i.e. SUMMARISE_INSTR in prompy.py should always be used, but it should include summary_prompt from user and partitions from user but also other scaffolding
    sample = None  # haven't done anything w/ this yet, prob just need to add limit=sample, when doing client.list_examples
